...
export default function AccessibleCard({ title, description, link }: AccessibleCardProps) {
  ...
  return (
    <article role="group" aria-labelledby={id} style={{ border: 'var(--border-thick)', boxShadow: 'var(--shadow-heavy)', padding: '16px', margin: '16px 0', backgroundColor: 'var(--digital-yellow)', color: 'var(--foundation-black)' }}>
      <h2 id={id}>{title}</h2>
      <p>{description}</p>
